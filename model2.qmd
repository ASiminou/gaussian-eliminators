...
```{r}
testing <- full_games_and_stats |>
  filter(Season == 2024)

training <- full_games_and_stats |>
  filter(Season != 2024)
```
...
```{r}
model2_predicted <- testing |>
  mutate(predicted = predict(model2, newdata = testing)) |>
  relocate(diff, predicted)
```
...
merged_final_2026 <- left_join(
  ...
  select(
    TeamName, opponent, location, Season, diff_eFGPct_offense, diff_TOPct_offense, diff_ORPct_offense, diff_FTRate_offense,
    diff_eFGPct_defense, diff_TOPct_defense, diff_ORPct_defense, diff_FTRate_defense
  )|>
...
```{r}
merged_final_2026_pred <- merged_final_2026 |>
  mutate(predicted = predict(model2, newdata = merged_final_2026)) |>
  mutate(predicted_actual = (.7543102 * predicted) + 1.2610274) |>
  relocate(TeamName, opponent, predicted_actual)
```
